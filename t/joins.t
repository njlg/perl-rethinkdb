...
# inner_join
$res = r->table('marvel')->inner_join(r->table('dc'), sub ($$) {
...
is $res->response->[0]->{left}->{superhero}, 'Captain America', 'Correct response';
is $res->response->[0]->{right}->{name}, 'Superman', 'Correct response';

# outer_join
$res = r->table('marvel')->outer_join(r->table('dc'), sub ($$) {
  my ($marvel_row, $dc_row) = @_;
  return $marvel_row->attr('strength')->lt($dc_row->attr('strength'));
})->run;
...
$res = r->table('marvel')->eq_join('dc_buddy', r->table('dc'), { index => 'name' })->run;

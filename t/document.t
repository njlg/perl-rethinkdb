...
r->table('marvel')->insert([
  { user_id => 1, superhero => 'Iron Man', superpower => 'Arc Reactor', active => 1, age => 35, villians => { count => 5 } },
  { user_id => 2, superhero => 'Hulk', superpower => 'Smash', active => 1, age => 35, villians => { count => 6 } },
  { user_id => 3, superhero => 'Captain America', superpower => 'Super Strength', active => 1, age => 135, villians => { count => 7 } },
  { user_id => 4, superhero => 'Thor', superpower => 'God-like powers', active => 1, age => 1035, villians => { count => 8 } },
  { user_id => 5, superhero => 'Hawk-Eye', superpower => 'Bow-n-arrow', active => 0, age => 35, villians => { count => 9 } },
  { user_id => 6, superhero => 'Wasp', superpower => 'Bio-lasers', active => 0, age => 35, villians => { count => 5 } },
  { user_id => 7, superhero => 'Ant-Man', superpower => 'Size', active => 1, age => 35, villians => { count => 10 } },
  { user_id => 8, superhero => 'Wolverine', superpower => 'Adamantium', active => 0, age => 35, villians => { count => 1 } },
  { user_id => 9, superhero => 'Spider-Man', superpower => 'Spidy Sense', active => 0, age => 20, villians => { count => 2 } },
])->run;
...
$res = r->table('marvel')->filter(r->row->attr('villians')->attr('count')->ge(10))->run;
...
$res = r->table('marvel')->filter(sub ($) {
  my $doc = shift;
  return $doc->attr('superhero')->eq(r->table('prizes')->get('winner')->attr('name'));
})->run;
...
$res = r->expr(['Iron Man', 'Spider-Man'])->splice_at(1, ['Hulk', 'Thor'])->run($conn);
...
$res = r->expr(['Iron Man', 'Hulk', 'Thor', 'Spider-Man'])->delete_at(1, 3)->run($conn);
...
$res = r->expr(['Iron Man', 'Bruce Banner', 'Thor'])->change_at(1, 'Hulk')->run($conn);

...
r->table('marvel')->insert([
  { user_id => 1, superhero => 'Iron Man', superpower => 'Arc Reactor', active => 1, age => 35, monsters => ['Wererabit'] },
  { user_id => 2, superhero => 'Hulk', superpower => 'Smash', active => 1, age => 35, monsters => ['Werewolf'] },
  { user_id => 3, superhero => 'Captain America', superpower => 'Super Strength', active => 1, age => 135, monsters => ['Werecat'] },
  { user_id => 4, superhero => 'Thor', superpower => 'God-like powers', active => 1, age => 1035, monsters => ['Weredog'] },
  { user_id => 5, superhero => 'Hawk-Eye', superpower => 'Bow-n-arrow', active => 0, age => 35, monsters => ['Werehound'] },
  { user_id => 6, superhero => 'Wasp', superpower => 'Bio-lasers', active => 0, age => 35, monsters => ['Wereelephant'] },
  { user_id => 7, superhero => 'Ant-Man', superpower => 'Size', active => 1, age => 35, monsters => ['Werebear'] },
  { user_id => 8, superhero => 'Wolverine', superpower => 'Adamantium', active => 0, age => 35, monsters => ['Werehampster'] },
  { user_id => 9, superhero => 'Spider-Man', superpower => 'Spidy Sense', active => 0, age => 20, monsters => ['Werepig', 'Werechicken'] },
])->run;
...
is_deeply [keys %{$res->response->[0]}], ['superhero', 'age'], 'Correct document fields';

# concat_map - Flattens a sequence of arrays returned by the mappingFunction into a single sequence.
$res = r->table('marvel')->concat_map(sub {
  my $hero = shift;
...
is $res->type, 2, 'Correct response type';
is_deeply $res->response, [
  'Werecat', 'Werepig', 'Werechicken', 'Werehampster', 'Werehound',
  'Werewolf', 'Werebear', 'Wererabit', 'Weredog', 'Wereelephant'
], 'Correct document fields';

# order_by
my $order1 = ['Ant-Man', 'Captain America', 'Hawk-Eye', 'Hulk', 'Iron Man', 'Spider-Man', 'Thor', 'Wasp', 'Wolverine'];
my $order2 = ['Spider-Man', 'Wolverine', 'Hawk-Eye', 'Wasp', 'Captain America', 'Hulk', 'Ant-Man', 'Iron Man', 'Thor'];
my $order3 = ['Captain America', 'Hulk', 'Ant-Man', 'Iron Man', 'Thor', 'Spider-Man', 'Wolverine', 'Hawk-Eye', 'Wasp'];
...
$res = r->table('marvel')->union(r->table('dc'))->order_by('popularity')->indexes_of(
  r->row->attr('superpowers')->contains('invisibility')
)->run;
